...
promptsRouter.post("/", (req, res) => {
  ...
  if (!body.title?.trim()) return res.status(400).json({ error: "Titel ist erforderlich." });
  if (!body.content?.trim()) return res.status(400).json({ error: "Prompt-Text ist erforderlich." });

  const stmt = db.prepare(`
    INSERT INTO prompts (title, content, area, tags, pinned)
  ...
  res.status(201).json(serialize(db.prepare("SELECT * FROM prompts WHERE id = ?").get(info.lastInsertRowid)));
});
...
promptsRouter.patch("/:id", (req, res) => {
  ...
  res.json(serialize(db.prepare("SELECT * FROM prompts WHERE id = ?").get(req.params.id)));
});
